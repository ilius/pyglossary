...
def imageFromFile(path: str, size: int = 0) -> gtk.Image:  # noqa: ARG001
	# the file must exist
	if not isabs(path):
		path = join(appResDir, path)
	im = gtk.Image()
	im.set_from_file(path)
	# im.set_from_pixbuf(pixbufFromFile(path, size=size))
	return im
...
# FIXME: Dialog is deprecated since version 4.10, Use Window instead
