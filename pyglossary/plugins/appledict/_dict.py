...
def indexes_generator(
        indexes_lang: str) -> Callable[[str, List[str], str, Any], str, ]:
    """
...
def format_clean_content(
    ...
    if BeautifulSoup:
        ...
        for tag in soup.select("[href]"):
            ...
            elif href.startswith("phonetics") or href.startswith(
                    "help:phonetics"):  # for oxford9
                # print(tag)
            ...
            # print
                      ...
                      or href.startswith("addid:") or href.startswith("addpv:")
                      or href.startswith("help:") or href.startswith("helpg:")
                      or href.startswith("helpp:") or href.startswith("helpr:")
                      or href.startswith("helpxr:") or href.startswith("xi:")
                      or href.startswith("xid:") or href.startswith("xp:")
                      or href.startswith("sd:") or href.startswith("#")):

                tag["href"] = f"x-dictionary:d:{href}"

        for thumb in soup.find_all('div', 'pic_thumb'):
            thumb[
                'onclick'] = f'this.setAttribute("style", "display:none"); this.nextElementSibling.setAttribute("style", "display:block")'
