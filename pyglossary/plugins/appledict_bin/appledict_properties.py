...
class AppleDictProperties:
	def __init__(
		...
		# in plist file: for "IDXDataFieldName" = "DCSExternalBodyID", see "IDXDataSize" = 4 or 8
		...
		# in plist file: key_text_metadata -> 'TrieAuxiliaryDataOptions' -> 'HeapDataCompressionType'
...
def from_metadata(metadata: Dict) -> AppleDictProperties:
	...
	entry_id_metadata = metadata.get('IDXDictionaryIndexes')[1]
	body_metadata = metadata.get('IDXDictionaryIndexes')[2]

	key_text_data_fields = key_text_metadata.get('IDXIndexDataFields')
	key_text_field_order = [field_data['IDXDataFieldName'] for field_data in key_text_data_fields.get('IDXVariableDataFields')]

	body_has_sections = key_text_data_fields.get("IDXExternalDataFields")[0].get("IDXDataSize") == 8
	...
	if 'TrieAuxiliaryDataOptions' in key_text_metadata and 'HeapDataCompressionType' in key_text_metadata['TrieAuxiliaryDataOptions']:
		key_text_compression_type = key_text_metadata['TrieAuxiliaryDataOptions']['HeapDataCompressionType']
	else:
