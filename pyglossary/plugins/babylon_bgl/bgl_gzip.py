...
def write32u(output: IO[bytes], value: int) -> None:
...
class _PaddedFile:
	...
	def __init__(self, f: IO[bytes], prepend: bytes = b"") -> None:
...
class GzipFile(_compression.BaseStream):
	...
	def __init__(
		...
		mode: str | None = None,
		compresslevel: int | None = _COMPRESS_LEVEL_BEST,
	...
	def _init_write(self, filename: str) -> None:
		self.name = filename
	...
	def flush(self, zlib_mode: int = zlib.Z_SYNC_FLUSH) -> None:
		self._check_not_closed()
		if self.mode == WRITE:
...
def _read_gzip_header(fp: IO[bytes]) -> bytes:
	"""
...
class _GzipReader(_compression.DecompressReader):
	...
	def _add_read_data(self, data: bytes) -> None:
