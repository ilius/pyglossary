...
class MemoryLoadedGlossary:
	"""In-memory `EntryList` backing for a glossary."""

	__slots__ = ()

	@staticmethod
	def newEntryList(
		entry_to_raw: Callable[[EntryType], RawEntryType],
		entry_from_raw: Callable[[RawEntryType], EntryType],
	) -> EntryList:
		return EntryList(
			entryToRaw=entry_to_raw,
			entryFromRaw=entry_from_raw,
		)


class SQLiteLoadedGlossary:
	"""SQLite-backed entry storage (on-disk or in-memory database)."""

	__slots__ = ()

	@staticmethod
	def newInMemoryList(glos: GlossaryCommon) -> SqEntryList:
		return SqEntryList(
			entryToRaw=glos._entryToRaw,
			entryFromRaw=glos._entryFromRaw,
			database="file::memory:",
			create=True,
		)

	@staticmethod
	def switchToFile(glos: GlossaryCommon, input_filename: str) -> None:
		sq_fpath = join(cacheDir, f"{os.path.basename(input_filename)}.db")
		if isfile(sq_fpath):
			log.info(f"Removing and re-creating {sq_fpath!r}")
			os.remove(sq_fpath)

		glos._data = SqEntryList(
			entryToRaw=glos._entryToRaw,
			entryFromRaw=glos._entryFromRaw,
			database=sq_fpath,
			create=True,
		)
		glos._cleanupPathList.add(sq_fpath)

		if not glos.alts:
			log.warning(
				"SQLite mode only works with enable_alts=True, force-enabling it.",
			)
		glos._config["enable_alts"] = True
		glos._sqlite = True


class GlossaryCreator:
	"""Builds entry objects and appends them to the glossary's entry list."""

	__slots__ = ("_glos",)

	def __init__(self, glos: GlossaryCommon) -> None:
		self._glos = glos

	def addEntry(self, entry: EntryType) -> None:
		self._glos._data.append(entry)
	...
	def newEntry(
		...
		glos = self._glos
		if not defiFormat:
			defiFormat = glos._defaultDefiFormat
	...
	def newDataEntry(self, fname: str, data: bytes) -> EntryType:
		...
		if glos._tmpDataDir:
			return DataEntry(
				fname,
				data,
				tmpPath=join(glos._tmpDataDir, fname.replace("/", "_")),
			)

		tmpDir = join(cacheDir, "tmp")
		os.makedirs(tmpDir, mode=0o700, exist_ok=True)
		glos._cleanupPathList.add(tmpDir)
		return DataEntry(
			fname,
			data,
...
class GlossaryCommon(GlossaryProgress):  # noqa: PLR0904
	"""
	The signature of `convert` differs from the legacy `glossary.Glossary` API;
	see `help(Glossary.convert)`.

	Metadata lives on `self.info` (`GlossaryInfo`), not on the glossary object itself.

	These methods do not exist here (but still exist in glossary.py):

		- read():
			use directRead() then iterate over the glossary

		- sortWords():
			sort entries yourself (when adding or after directRead)
	...
	def _newInMemorySqEntryList(self) -> SqEntryList:
		return SQLiteLoadedGlossary.newInMemoryList(self)
	...
	def __init__(
		...
		"""
		self.info = GlossaryInfo()
		GlossaryProgress.__init__(self, ui=ui)
		self._creator = GlossaryCreator(self)
		self._convertor = GlossaryConvertor(self)
		self._config: ConfigType = {}
		self._data: EntryListType = MemoryLoadedGlossary.newEntryList(
			self._entryToRaw,
			self._entryFromRaw,
		)
		self._sqlite = False
		...
		if info:
			...
			warnings.warn(
				"info= argument is deprecated. Use glos.info[key] = value",
	...
	def __str__(self) -> str:
		name = self.info._info.get(c_name)
		return f"Glossary{{filename: {self._filename!r}, name: {name!r}}}"
	...
	def newEntry(
		...
		defiFormat must be empty or one of these:
				...
				"x": xdxf
		"""
		return self._creator.newEntry(word, defi, defiFormat, byteProgress)

	def newDataEntry(self, fname: str, data: bytes) -> EntryType:
		return self._creator.newDataEntry(fname, data)
	...
	def _openReader(self, reader: Any, filename: str) -> None:
		...
		hasTitleStr = self.info["definition_has_headwords"]
		if hasTitleStr:
	...
	def _read(
		...
		**options: Any,
		...
		if not self.info.name:
			self.info.name = os.path.split(filenameUC)[1]
	...
	def _resolveSortParams(
		...
		if sqlite:
			SQLiteLoadedGlossary.switchToFile(self, args.inputFilename)
	...
	def convertV2(self, args: ConvertArgs) -> str:
		return self._convertor.convertV2(args)


# ________________________________________________________________________#


class GlossaryConvertor:
	"""High-level read → metadata → write conversion pipeline."""
	...
	@staticmethod
	def _validateArgs(args: ConvertArgs) -> None:
	...
	def _convertPrepare(
		...
		glos = self._glos
		if isdir(output_filename) and os.listdir(output_filename):
			raise Error(
				f"Directory already exists and not empty: {relpath(output_filename)}",
			)

		direct, sort = glos._resolveSortParams(
			args=args,
			plugin=PluginHandler.plugins[output_format],
		)

		showMemoryUsage()

		read_options = args.readOptions or {}
	...
	def convertV2(self, args: ConvertArgs) -> str:
		...
		output_filename, output_format, compression = PluginHandler.detectOutputFormat(
		...
		if compression and not PluginHandler.plugins[output_format].singleFile:
			os.makedirs(output_filename, mode=0o700, exist_ok=True)

		write_options = args.writeOptions or {}

		final_output_file = glos._write(
			output_filename,
		...
		if compression:
			final_output_file = GlossaryCommon._compressOutput(
				final_output_file,
				compression,
			)

		log.info(f"Writing file {final_output_file!r} done.")
