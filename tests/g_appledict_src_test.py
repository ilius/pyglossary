...
class TestGlossaryAppleDictSrc(TestGlossaryBase):
	def __init__(self, *args, **kwargs):
		TestGlossaryBase.__init__(self, *args, **kwargs)

		self.dataFileCRC32.update({
			"appledict-src/002-no-morphology.txt": "4afadab4",
			"appledict-src/002-no-morphology-appledict-src-expected.xml": "60044894",
		})
	...
	def test_tabfile_without_morpho_to_appledict_source(self):
		...
		expectedOutputFilePath = self.downloadFile(f"appledict-src/{baseName}-appledict-src-expected.xml")
